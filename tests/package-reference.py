...
root=Path.cwd()
out=root.parent/'IdeaNova_Reference_Match_2026-09-12.zip'
...
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
    for p in root.rglob('*'):
        ...
        if rel.parts[0]=='qa':
            if not (len(rel.parts)>2 and rel.parts[1]=='reference-match'): continue
            ...
            if p.name not in qa_names and not is_comparison: continue
...
with zipfile.ZipFile(out) as z:
    ...
    assert 'ideanova/qa/reference-match/comparison/index.html' in z.namelist()
    assert 'ideanova/docs/REFERENCE_MATCH.md' in z.namelist()
    assert 'ideanova/docs/REFERENCE_MATCH_VALIDATION.md' in z.namelist()
