...
root=Path.cwd()
out=root.parent/'IdeaNova_Seam_Repair_2026-09-12.zip'
...
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
    for p in root.rglob('*'):
        ...
        if rel.parts[0]=='qa':
            if not (len(rel.parts)>2 and rel.parts[1]=='seam-repair'): continue
            ...
            if rel.parts[2]=='after' and p.suffix=='.png' and 'x' not in p.stem and p.name!='keyboard-skip.png': continue
...
with zipfile.ZipFile(out) as z:
    ...
    assert 'ideanova/qa/seam-repair/comparison/index.html' in z.namelist()
    ...
    assert 'ideanova/qa/seam-repair/seams.json' in z.namelist()
