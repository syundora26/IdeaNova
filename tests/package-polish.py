...
root=Path.cwd()
out=root.parent/'IdeaNova_Reference_Polish_2026-09-12.zip'
...
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
    for p in root.rglob('*'):
        ...
        if rel.parts[0]=='qa':
            if not (len(rel.parts)>2 and rel.parts[1]=='reference-polish'): continue
            ...
            if rel.parts[2]=='source-before' or 'draft' in p.name or p.name=='failure.png': continue
...
with zipfile.ZipFile(out) as z:
    ...
    assert 'ideanova/qa/reference-polish/comparison/index.html' in z.namelist()
    assert 'ideanova/docs/REFERENCE_POLISH.md' in z.namelist()
    assert 'ideanova/public/images/reference-stone-extension.png' in z.namelist()
    assert 'ideanova/qa/reference-polish/comparison/review.html' in z.namelist()
    assert 'ideanova/docs/REFERENCE_POLISH_VALIDATION.md' in z.namelist()
