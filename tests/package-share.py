...
root=Path.cwd()
out=root.parent/'IdeaNova_Share_Preview_Source_2026-09-12.zip'
...
with zipfile.ZipFile(out,'w',zipfile.ZIP_DEFLATED,compresslevel=6) as z:
    for p in root.rglob('*'):
        ...
        if rel.parts[0]=='qa':
            if not (len(rel.parts)>2 and rel.parts[1] in {'lower-finish','contact-contour','share-preview'}): continue
...
with zipfile.ZipFile(out) as z:
    ...
    assert 'ideanova/docs/SHARE_PREVIEW.md' in z.namelist()
    assert 'ideanova/vite.share.config.ts' in z.namelist()
    assert 'ideanova/src/share/gateway.ts' in z.namelist()
