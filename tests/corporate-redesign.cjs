...
const dir = 'qa/corporate-redesign';
  ...
  try {
    for (const [width,height] of [[1440,1000],[768,1024],[390,844],[320,740]]) {
      ...
      for (const route of routes) {
        ...
        if (route==='/') {
          ...
          const fonts=await page.locator('.corporate-hero h1').evaluate(el=>({size:getComputedStyle(el).fontSize,height:el.getBoundingClientRect().height,lineHeight:getComputedStyle(el).lineHeight}));
          results.typography.push({width,...fonts});
          expect(Math.round(fonts.height/parseFloat(fonts.lineHeight))).toBe(2);
          if(width===1440 || width===390) for (const section of ['corporate-business-section','corporate-columns','corporate-shopping','corporate-contact']) {
            await page.locator('.'+section).screenshot({path:dir+'/'+width+'-'+section+'.png'});
          }
        }
      }
    ...
    for (const state of ['loading','empty','error']) {
      ...
      if(state==='loading') {
        ...
        await page.screenshot({path:dir+'/390-articles-loading.png',fullPage:true}); release(); await expect(page.getByText('現在、掲載中の記事はありません。')).toHaveCount(2);
      } else if(state==='empty') {
        await expect(page.getByText('現在、掲載中の記事はありません。')).toHaveCount(2);
      ...
      if(state==='error') {
        ...
        await expect(page.locator('.corporate-news-section .corporate-article')).toHaveCount(1);
      }
      await ready(page,base+'/');
    }
    ...
    for (const url of [base+'/shop',base+'/shop/agriculture',base+'/shop/login','http://127.0.0.1:5179/','http://127.0.0.1:5179/contact',base+'/admin']) {
      ...
      await page.evaluate(()=>{for(const s of document.styleSheets)if(s.ownerNode?.getAttribute('data-vite-dev-id')?.endsWith('/corporate.css'))s.disabled=true;});
    ...
    await page.locator('.corporate-shopping').getByRole('link',{name:'化粧品オンラインショップへ',exact:true}).click();
    ...
    await page.locator('.corporate-shopping').getByRole('link',{name:'農作物ショップへ',exact:true}).click();
    await expect(page.locator('.shop-nav').getByRole('link',{name:'カート 1点'})).toBeVisible();
