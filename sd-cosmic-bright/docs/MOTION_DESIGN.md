...
| 7 | 追加要望 | [07-planet-rotation.md](motion/07-planet-rotation.md) — 惑星の自転（H8、WebGL）、輪の粒の群れ（H7'）、背景の宇宙のアニメーション（U2） |
...
| H8 | hero | **惑星が自転**: 絵を模様と陰影に分離し、WebGL で模様だけを 110 秒周期で回す（縁の光・輪は固定、開始時は元の絵と一致） | 常時（ヒーロー可視時） | なし（静止画） |
| U2 | 背景 | 星雲の光 2 つの漂いと明滅、色被せの呼吸、星空と星の層の左右の揺れ、流れ星 9〜19 秒間隔 | 常時 | なし |
| H6 | hero | 惑星画像の夜空 30 か所で星が瞬く（文字と重なる位置は自動で非表示） | 常時（ヒーロー可視時） | なし |
| H7 | hero | 輪の外縁の 4 つの光の粒（44 秒）+ 帯の中を回る 36 個の小さな粒（38〜48 秒、内側ほど速い）。奥側は減光、惑星の裏では非表示 | 常時（ヒーロー可視時） | なし |
